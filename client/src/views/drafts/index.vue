<template>
  <div>
    <PageHeader :back="true" title="草稿箱"></PageHeader>
    <PageContainer>
      <div class="list-box">
...
<script>
  ...
  export default {
    ...
    methods: {
      async getDrafts() {
        let res = await $axios('/getDrafts', {currentPage: this.currentPage, pageSize: this.pageSize});
        if (res.code == 200) {
...
//   发表
      ...
      async publish(item){
        let res = await $axios('/publishDrafts',{articleId:item.id})
        if (res.code == 200){
